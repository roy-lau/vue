                    ...
                    <svg class="icon icon-ok">
                      <use xlink:href="#icon-ok"></use>
                                        ...
                                        <div class="cart-tab-5">
                                            <div class="cart-item-opration" @click="delCartConfirm(item.productId,item.productNum)">
...
export default {
    ...
    methods: {
        ...
        editCart(flag, item) {
            ...
            this.$axios.post('users/cart/edit', item).then(res => {
                let data = res.data,
                    num = 0
                if (flag==='add') {
                    num = 1
        ...
        toggleCheckAll() {
            ...
            this.$axios.post('cart/edit/checkAll', { checkAll: flag }).then(res => {
                // let data = res.data
            })
        },
        // 删除购物车的弹出框
        ...
        delCat() {
            ...
            this.$axios.post('users/cart/del', { productId: this.productId }).then(res => {
                let data = res.data
                if (!data.status) {
                    this.modalConfirm = false
